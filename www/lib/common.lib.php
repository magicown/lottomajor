    ...
    while($total<$mb_cnt){
        ...
        if (!isset($chk[$rand])) {
            ...
            if($filter['chain_is_1']=='Y') {//연번 모두 제외한다.
                $que = "SELECT idx, no1,no2,no3,no4,no5,no6 FROM lotto_num WHERE idx = {$rand} AND use_yn = 'N' AND chain_is_1 = 'N' AND chain_is_2 = 'N' AND chain_is_3 = 'N' AND chain_is_4 = 'N' AND chain_is_5 = 'N' ";
            } else {
                //1연번 포함한다.
...
function makeLottoNumPayOne($except, $sms_cnt, $today_cnt, $mb_id, $turn, $sel_cnt){
    ...
    /*echo $remind_sms_cnt;//현재 남은 문자 갯수
    echo "<br>";*/
...
function SendMesg($rev_hp, $rev_msg) {
    ...
    $url = 'https://rest.supersms.co/sms/xml?'.http_build_query($query);
...
function makeFreeLottoNum($mb_id, $mb_hp){
    ...
    if($mb_hp) {
        $msg = "{$turn}회차 1등 예상번호 로또메이저 체험플랜\n{$sm}\n\n무료거부 (080-855-6398)";
        // 한줄로 이어쓰기 하세요.
