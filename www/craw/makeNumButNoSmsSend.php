...
while ($pay = sql_fetch_array($res_que)) {
    ...
    //echo $sql."<br>";
    $row = sql_fetch($sql);
    echo "전송횟수는->".$row['cnt']."<br><br>";
    //echo "<br>";
    if($row['cnt']==0 || !$row['cnt']){
        //echo $pay['mb_id']."-".$pay['mb_hp']."<br>";
        //등록된 번호를 불러온다.
        ...
        if($num_chk['ccnt']>0) {
            ...
            echo $hpMesg = "{$turn}회차 1등 예상번호 로또메이저 {$pay_name['pay_name']}\n{$sm}\n\n무료거부 (080-855-6398)";
            echo "<br><br>";
            if($pay_name['mb_5']!='Y') {//일시정지 회원은 문자발송 안한다.
