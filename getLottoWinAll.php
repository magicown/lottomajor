...
include("/home/lottomajor/Snoopy.class.php");
...
for($i=1;$i<=852;$i++) {
    $snoopy->fetch("https://dhlottery.co.kr/gameResult.do?method=byWin&drwNo=".$i);
    $content = $snoopy->results;

    preg_match_all('/<strong>([0-9]{1,10})/isx', $content, $hit_turn);
//print_r($hit_turn);

    preg_match_all('/<span\sclass="[^>]+>([0-9]{1,2})<\/span>/isx', $content, $ball);
//print_r($ball);

    preg_match_all('/<p\sclass="desc">(.*?)<\/p>/isx', $content, $hit_date);
    print_r($hit_date);
    $year = substr($hit_date[1][0],1,4);
    $month = substr($hit_date[1][0],8,2);
    $day = substr($hit_date[1][0],13,2);
...
//print_r($etc);
    ...
    if (!$row[cnt]) {
        ...
        $sql .= "hit_date = '{$year}-{$month}-{$day} 20:50:00', ";
